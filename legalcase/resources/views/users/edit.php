...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-slate-800">Edit User</h2>
                <p class="text-slate-500 text-sm mt-0.5"><?= clean($user['email']) ?></p>
            </div>
            <a href="<?= APP_URL ?>/users" class="text-slate-400 hover:text-slate-600 text-sm">← Back to users</a>
        </div>
        <form method="POST" action="<?= APP_URL ?>/users/<?= $user['id'] ?>/update" class="px-6 py-5 space-y-5">
            <?= csrf_field() ?>
